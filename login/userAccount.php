...
if(isset($_POST['signupSubmit'])){
    ...
    if(!empty($_POST['email'])){
		...
		if($prevUser > 0){
			...
			if($update){
				$resetPassLink = 'http://localhost/login/resetPassword.php?fp_code='.$uniqidStr;
				...
				//additional headers
				$headers .= 'From: Chiquimundi<sender@example.com>' . "\r\n";
				//send email
				mail($to,$subject,$mailContent,$headers);
				
				$sessData['status']['type'] = 'success';
                $sessData['status']['msg'] = 'Verifique su correo electrónico, hemos enviado un enlace para restablecer su contraseña.';
			}else{
				$sessData['status']['type'] = 'error';
				$sessData['status']['msg'] = 'Se produjo algún problema, por favor intente nuevamente.';
			}
	...
	if(!empty($_POST['password']) && !empty($_POST['confirm_password']) && !empty($_POST['fp_code'])){
        ...
        if($_POST['password'] !== $_POST['confirm_password']){
            $sessData['status']['type'] = 'error';
            $sessData['status']['msg'] = 'Verifique que sean las mismas contraseñas.'; 
        }else{
